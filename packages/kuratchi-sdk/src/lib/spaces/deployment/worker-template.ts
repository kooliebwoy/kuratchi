/**
 * Kuratchi Spaces - Worker Template
 * Durable Object worker script for chat spaces with SQLite storage
 */
...
export class KuratchiChatSpace extends DurableObject {
  ...
  initializeSchema() {
    ...
    this.sql.exec(\`CREATE INDEX IF NOT EXISTS idx_messages_created_at ON messages(created_at)\`);
    this.sql.exec(\`
