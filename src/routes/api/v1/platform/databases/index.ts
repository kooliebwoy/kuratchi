...
export async function GET(ctx: RouteContext): Promise<Response> {
  ...
  const result = await db.databases.where({ isActive: true, organizationId: auth.organizationId }).many();
...
export async function POST(ctx: RouteContext): Promise<Response> {
  ...
  const { createDatabase } = await import('$server/database/databases');
  try {
    await createDatabase(form);
    const result = await db.databases.where({ name: body.name, isActive: true, organizationId: auth.organizationId }).first();
    return jsonResponse({ success: true, data: result.data });
  } catch (e: any) {
