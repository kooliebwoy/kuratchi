...
export class StripeService {
  ...
  async setupOrganizationBilling(options: BillingSetupOptions): Promise<{
    ...
    const orgRes = await this.client.organizations.where({ id: options.organizationId } as any).first();
  ...
  async getOrganizationWithBilling(organizationId: string): Promise<any> {
    const orgRes = await this.client.organizations.where({ id: organizationId } as any).first();
    const organization = (orgRes as any)?.data;
