...
export class KuratchiD1v2 {
  ...
  : Promise<{ database: any; token: string }> {
    ...
    if (opts.migrate) {
      if (!opts.schema) throw new Error('KuratchiD1v2.createDatabase: migrate:true requires a schema');
  ...
  client(
    ...
    if (!options?.schema) throw new Error('KuratchiD1v2.client requires a schema (DatabaseSchema or SchemaDsl)');
  ...
  database(cfg: { databaseName: string; dbToken: string; gatewayKey: string }) {
    return {
      ...
      client: (options: { schema: DatabaseSchema | SchemaDsl }): Record<string, TableApi> => {
        const exec = (sql: string, params?: any[]) => this.getClient(cfg).query(sql, params);
        if (!options?.schema) throw new Error('KuratchiD1v2.database().client requires a schema (DatabaseSchema or SchemaDsl)');
        const normalized = ensureDbSchema(options.schema);
        return createClientFromJsonSchema(exec, normalized);
      },
    };
