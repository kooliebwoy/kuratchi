...
- Use `InferSchema<(typeof MyDO.schemas).methodName>` for the typed method parameter.

Example with defaults and arrays:
...
Files under `src/server/**/*.agents.ts` are treated as first-class Worker exports.
...
Agent files are re-exported from `.kuratchi/worker.ts`, but Wrangler still needs matching bindings and migrations.
