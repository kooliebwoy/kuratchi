...
Environment bindings and request `locals` (e.g. the authenticated user)
are reached through `getEnv()` / `getLocals()` from `@kuratchi/js` rather
than being threaded through the action signature — one canonical
accessor keeps `$server/*` modules portable between render, RPC, and
action code.
...
Button-triggered actions share the same transport as `<form action={fn}>`:
a regular `POST` to the current route with `_action` (the function name)
and `_args` (a JSON-encoded array of the call arguments). The framework
emits a client bridge that serializes the click-site arguments and
submits them as a real form, so button actions retain the same
progressive-enhancement and POST-Redirect-GET properties as forms. On
success the framework returns a 303 redirect back to the current URL,
so the browser re-renders with fresh data. Return a `Response` from your
action to override that behavior.
...
<Note>
  ...
  `await deleteItem(id)` inside `onDelete`), Kuratchi dispatches it
...
See [Client interactivity](/framework/client-interactivity) for the full
