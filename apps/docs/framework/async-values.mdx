...
For Cloudflare Workflow instance status — which changes over time — use [`workflowStatus`](/framework/workflows#live-status-with-workflowstatus) from `kuratchi:workflow`. It returns the same `AsyncValue` shape and, when `{ poll }` is passed, the framework re-renders the route on each tick.
