<script lang="ts">
  ...
  import { ArrowLeft, Save, Eye, Image, X, Trash2 } from '@lucide/svelte';
  ...
  function loadPost() {
    ...
    const posts: (BlogPost & { blocks?: BlockSnapshot[] })[] = JSON.parse(saved);
  ...
  async function savePost(isAutoSave = false) {
    ...
    try {
      // Get existing posts
      const saved = localStorage.getItem('kuratchi_blog_posts');
      let posts: (BlogPost & { content?: string; blocks?: BlockSnapshot[] })[] = saved ? JSON.parse(saved) : [];
  ...
  function deletePost() {
    if (confirm('Are you sure you want to delete this post?')) {
      ...
      if (saved) {
        ...
        localStorage.setItem('kuratchi_blog_posts', JSON.stringify(filtered));
      }
      goto('/blog');
    }
  }
...
{#if isLoading}
  <div class="kui-loading">
    <div class="kui-spinner"></div>
    <p>Loading post...</p>
  </div>
{:else if notFound}
  <div class="kui-not-found">
    ...
    <a href="/blog" class="kui-btn kui-btn--primary">Back to Posts</a>
  </div>
{:else}
  <div class="kui-post-editor">
    ...
    <header class="kui-post-editor__header">
      ...
      <div class="kui-post-editor__header-right">
        ...
        {/if}
        <button type="button" class="kui-btn kui-btn--ghost kui-btn--danger" onclick={deletePost}>
          <Trash2 />
          <span>Delete</span>
        </button>
...
<style>
  ...
  .kui-spinner {
    width: 32px;
    height: 32px;
    border: 3px solid var(--kui-color-border);
    border-top-color: var(--kui-color-primary);
    border-radius: 50%;
  ...
  .kui-btn--ghost.kui-btn--danger {
    color: var(--kui-color-danger, #ef4444);
    border-color: var(--kui-color-danger, #ef4444);
  }

  .kui-btn--ghost.kui-btn--danger:hover {
