...
<div class="kui-billing">
	...
	<Card class="kui-panel kui-current-plan">
		<div class="kui-current-plan__top">
			<div class="kui-inline">
				...
				<div>
					<h3 class="kui-plan-title">
						{subscriptionData?.planName || subscriptionData?.plan || 'Free'} Plan
					</h3>
					{#if subscriptionData?.hasSubscription}
						<div class="kui-inline">
		...
		{:else}
			<p class="kui-subtext">Choose a plan below to get started</p>
		{/if}
	</Card>
	...
	{#if plansData.length > 0}
		<Card class="kui-panel" id="plans">
			<div class="kui-panel__header">
				...
				{#if subscriptionData?.hasSubscription}
					<span class="kui-subtext"
						>Current plan: {subscriptionData.planName || subscriptionData.plan}</span
					>
				{/if}
			</div>
			<div class="kui-plan-grid">
				{#each plansData as plan, index}
					...
					<Card
						class={`kui-plan ${isPopular ? 'is-popular' : ''} ${isCurrentPlan ? 'is-current' : ''}`}
					>
						{#if isPopular}
							<Badge variant="primary" size="xs" class="kui-plan__badge"
								>Most Popular</Badge
							>
						{/if}
						{#if isCurrentPlan}
							<Badge variant="success" size="xs" class="kui-plan__badge"
								>Current Plan</Badge
							>
						{/if}
						<div class="kui-inline">
						...
						{#if primaryPrice}
							<form {...upgradePlan}>
								...
								<Button
									...
									aria-busy={pendingPriceId === primaryPrice.id &&
										!!upgradePlan.pending}
	...
	<Card class="kui-panel">
		...
		<form {...updateBillingEmail} class="kui-stack">
			<FormField
				label="Email for invoices and receipts"
				issues={updateBillingEmail.fields.billingEmail.issues()}
			>
				<FormInput
					field={updateBillingEmail.fields.billingEmail}
					type="email"
...
<style>
	...
	.kui-current-plan {
		...
		border: 1px solid
			color-mix(in srgb, var(--kui-color-primary) 30%, var(--kui-color-border) 70%);
	}
	...
	.kui-plan.is-popular {
		border: 1px solid
			color-mix(in srgb, var(--kui-color-primary) 40%, var(--kui-color-border) 60%);
		box-shadow: var(--kui-shadow-sm);
	}

	.kui-plan.is-current {
		border: 1px solid
			color-mix(in srgb, var(--kui-color-success) 40%, var(--kui-color-border) 60%);
	}
