<script lang="ts">
	...
	import { getOems, createOem, updateOem, deleteOem } from '$lib/functions/catalog.remote';
	...
	async function handleAdd() {
		...
		try {
			...
			// Refresh OEMs to ensure consistency
			await oems.refresh();
			closeModals();
		} catch (err: any) {
	...
	{:else}
		<div class="oems-grid">
			{#each filteredOems() as oem}
				{@const vehicleCount = oem.vehicle_count || 0}
				<Card class="oem-card">
