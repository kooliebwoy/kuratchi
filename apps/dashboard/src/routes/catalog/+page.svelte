<script lang="ts">
	...
	import {
		...
		updateVehicle,
		scrapeVehicleUrl,
		importScrapedVehicle,
		getCategories,
		type CatalogCategory as Category,
	...
	const vehiclesData = $derived(vehicles.current?.data || []);
	const totalVehicles = $derived(vehicles.current?.total || 0);
	const totalPages = $derived(vehicles.current?.totalPages || 1);
	const currentPage = $derived(vehicles.current?.page || 1);
	...
	const selectedOem = $derived(
		selectedOemId ? oemsData.find((o: any) => o.id === selectedOemId) : null
	);
	const currentCategory = $derived(page.url.searchParams.get('category') || 'all');
	...
	// Sync searchQuery with URL when it changes externally
	$effect(() => {
	...
	// Categories with counts (calculated server-side now)
	const availableCategories = $derived.by(() => {
		// Add "All Vehicles" at the beginning
		// Note: Total count for "All" effectively comes from separate query or summing up,
		// but vehicle.remote handles counts per category.
		// For "All Vehicles" count, we can use `totalVehicles` ONLY if no other filters applied.
		// Ideally `getCategories` should return total count.
		// For now we'll sum the category counts or just use empty count for "All".
		const allCount = categoriesData.reduce(
			(acc: number, cat: any) => acc + (cat.vehicle_count || 0),
			0
		);
	...
	// Use vehiclesData directly (it's already paginated/filtered)
	const displayedVehicles = $derived(vehiclesData);
	...
	async function saveVehicleChanges() {
		...
		try {
			...
			await updateVehicle({
				...
				modelYear: editableVehicle.model_year
					? parseInt(editableVehicle.model_year)
					: undefined,
	...
	<div class="main-content">
		...
		{:else if viewMode === 'grid'}
			...
			<div class="vehicles-grid">
				{#each vehiclesData as vehicle}
					<div
						class="vehicle-card-wrapper"
						onclick={() => openVehicleDrawer(vehicle)}
						role="button"
						tabindex="0"
						onkeydown={(e) => e.key === 'Enter' && openVehicleDrawer(vehicle)}
					>
							...
							<div class="vehicle-content">
								...
								<div class="vehicle-meta">
									...
									<span class="vehicle-price"
										>{formatPrice(vehicle.msrp, vehicle.currency)}</span
									>
								</div>
							</div>
						</Card>
    ...
    {#if totalPages > 1}
        <div class="pagination-controls">
            ...
            <div class="pagination-info">
                ...
                <span class="pagination-total">({totalVehicles} items)</span>
            </div>
            <Button 
                variant="ghost" 
                size="sm" 
			...
			<div class="modal-content">
				...
				{#if activeTab === 'url'}
					<div class="modal-section">
						<p class="modal-text">
							Paste an OEM product page URL and we'll automatically extract the
							vehicle data.
						</p>
						...
						{#if scrapedData}
							<div class="modal-card">
								...
								<div class="modal-card-content">
									<div class="scrape-preview">
										...
										<div class="preview-info">
											<h4 class="preview-title">
												{scrapedData.modelName || 'Unknown Model'}
											</h4>
											{#if scrapedData.msrp}
												<p class="preview-price">
													{formatPrice(scrapedData.msrp)}
												</p>
											{/if}
									...
									<div class="modal-field">
										<label for="oem-select" class="modal-label"
											>Select OEM</label
										>
										<select
											id="oem-select"
											class="modal-input"
											bind:value={selectedOemForImport}
											required
										>
										...
										{#if oemsData.length === 0}
											<p class="modal-help-text warning">
												No OEMs found. Create an OEM first to import
												vehicles.
											</p>
										{/if}
									</div>
									...
									<div class="modal-actions">
										...
										<Button
											...
											disabled={importSubmitting || !selectedOemForImport}
											class="flex-1"
										>
											{#if importSubmitting}
...
{/if}
	...
	>
			...
			{#if selectedVehicle}
				...
				{#if getThumbnailUrl(selectedVehicle)}
					<div class="drawer-image">
						<img
							src={getThumbnailUrl(selectedVehicle)}
					...
					{#if isEditingVehicle}
						...
						<div class="edit-form">
							<div class="form-group">
								...
								<select
									id="category"
									bind:value={editableVehicle.category}
									class="form-input"
								>
							...
							<div class="form-group">
								...
								<select
									id="status"
									bind:value={editableVehicle.status}
									class="form-input"
								>
							...
							<div class="form-row">
								...
								<div class="form-group">
									...
									<select
										id="currency"
										bind:value={editableVehicle.currency}
										class="form-input"
									>
					...
					{:else}
						...
						<div class="drawer-badges">
							<Badge variant="neutral"
								>{getCategoryLabel(selectedVehicle.category)}</Badge
							>
							<Badge variant={getStatusColor(selectedVehicle.status)}
								>{selectedVehicle.status}</Badge
							>
		...
		{#snippet footer()}
			{#if isEditingVehicle}
				<!-- Edit Mode Actions -->
				<Button variant="ghost" onclick={cancelEditingVehicle}>Cancel</Button>
	...
	{#if showDeleteModal && selectedVehicle}
		<Dialog bind:open={showDeleteModal} size="sm" onClose={closeDeleteModal}>
			...
			{#snippet children()}
				<div class="space-y-4">
					<p>
						Are you sure you want to delete <strong>{selectedVehicle.model_name}</strong
						>? This action cannot be undone.
					</p>
					<div class="flex gap-2">
						<Button variant="ghost" onclick={closeDeleteModal} class="flex-1"
							>Cancel</Button
						>
						<Button
							variant="error"
...
<style>
	...
	.vehicle-placeholder.processing {
		background: linear-gradient(
			135deg,
			var(--kui-color-surface) 0%,
			var(--kui-color-muted-bg) 100%
		);
	}
