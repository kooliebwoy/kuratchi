...
async function getCachedAdminDb(locals: any) {
	return await (locals.kuratchi as any)?.getAdminDb?.();
}

async function getCachedOrgDb(locals: any, orgId: string) {
	return await (locals.kuratchi as any)?.orgDatabaseClient?.(orgId);
}
