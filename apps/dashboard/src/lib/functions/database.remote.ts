...
// Guarded command helper - for imperative actions with parameters
const guardedCommand = <R>(
	schema: v.BaseSchema<unknown, unknown, v.BaseIssue<unknown>>,
	fn: (data: any) => Promise<R>
) => {
...
// Get table data for a specific table (as a command - can be called with parameters)
export const getTableData = guardedCommand(
	v.object({
		tableName: v.pipe(v.string(), v.nonEmpty()),
		databaseId: v.pipe(v.string(), v.nonEmpty())
	}),
	async ({ tableName, databaseId }) => {
		const { locals } = getRequestEvent();
		
		try {
			const adminDb = await (locals.kuratchi as any)?.getAdminDb?.();
			if (!adminDb) {
				console.error('[getTableData] Admin database not available');
				return { rows: [], schema: [] };
			}
			...
			if (!db || !db.id) {
				console.error('[getTableData] Database not found:', databaseId);
				return { rows: [], schema: [] };
			}
			...
			if (!tokens || tokens.length === 0) {
				console.warn('[getTableData] No tokens found for database:', databaseId);
				return { rows: [], schema: [] };
			}
			...
			if (!gatewayKey) {
				console.error('[getTableData] KURATCHI_GATEWAY_KEY not found in environment');
				return { rows: [], schema: [] };
			}
			
			console.log('[getTableData] Connecting to database:', {
				databaseName: db.name,
				tableName,
				hasToken: !!tokens[0].token,
				hasGatewayKey: !!gatewayKey
			...
			// Get table schema
			const schemaResult = await dbQuery(`PRAGMA table_info(${tableName})`);
...
export const insertTableRow = guardedCommand(
	v.object({
		tableName: v.pipe(v.string(), v.nonEmpty()),
		databaseId: v.pipe(v.string(), v.nonEmpty()),
		rowData: v.record(v.string(), v.any())
	}),
	async ({ tableName, databaseId, rowData }) => {
		const { locals } = getRequestEvent();
		
		try {
			const adminDb = await (locals.kuratchi as any)?.getAdminDb?.();
			if (!adminDb) {
				console.error('[insertTableRow] Admin database not available');
			...
			if (!db || !db.id) {
				console.error('[insertTableRow] Database not found:', databaseId);
				error(404, 'Database not found');
			}
			...
			if (!tokens || tokens.length === 0) {
				console.warn('[insertTableRow] No tokens found for database:', databaseId);
				error(404, 'Database token not found');
			}
			...
			if (!gatewayKey) {
				console.error('[insertTableRow] KURATCHI_GATEWAY_KEY not found in environment');
				error(500, 'Gateway key not configured');
			}
			...
			console.error('[insertTableRow] Error:', err);
			error(500, 'Failed to insert row');
		}
	}
);

// Delete a row from a table
export const deleteTableRow = guardedCommand(
	v.object({
		tableName: v.pipe(v.string(), v.nonEmpty()),
		databaseId: v.pipe(v.string(), v.nonEmpty()),
		whereClause: v.pipe(v.string(), v.nonEmpty()),
		whereValues: v.array(v.any())
	}),
	async ({ tableName, databaseId, whereClause, whereValues }) => {
		const { locals } = getRequestEvent();
		
		try {
			const adminDb = await (locals.kuratchi as any)?.getAdminDb?.();
			if (!adminDb) {
				console.error('[deleteTableRow] Admin database not available');
				error(500, 'Admin database not available');
			}
			...
			if (!db || !db.id) {
				console.error('[deleteTableRow] Database not found:', databaseId);
				error(404, 'Database not found');
			}
			...
			if (!tokens || tokens.length === 0) {
				console.warn('[deleteTableRow] No tokens found for database:', databaseId);
				error(404, 'Database token not found');
			}
			...
			if (!gatewayKey) {
				console.error('[deleteTableRow] KURATCHI_GATEWAY_KEY not found in environment');
				error(500, 'Gateway key not configured');
			}
			...
			await dbQuery(sql, whereValues);

			return { success: true };
		} catch (err) {
			console.error('[deleteTableRow] Error:', err);
			error(500, 'Failed to delete row');
		}
	}
);
